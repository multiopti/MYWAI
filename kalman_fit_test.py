...
for k in range(ns):
    U[k,0] = np.sin(0.05*k)
    U[k,1] = np.cos(0.05*k)
    Y[k, 0] = np.sin(0.05 * (k+1)) + np.random.normal(0, stdev)
    Y[k, 1] = np.cos(0.05 * (k+1)) + np.random.normal(0, stdev)


knn_ekf = KNN(nu=2, ny=2, nl=10, neuron='logistic')
...
# Evaluation
Yp = knn_ekf.feedforward(U)
plt.plot(U[:,0], U[:,1], c='b' )
plt.scatter(Yp[:,0], Yp[:,1], c='r', s=5)
plt.grid(True)
plt.show()
